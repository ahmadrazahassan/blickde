...
export const metadata: Metadata = {
  ...
  alternates: { canonical: "/software-im-direktvergleich" },
};
...
export default async function ComparisonHubPage({ searchParams }: { searchParams: Promise<{ a?: string; b?: string }> }) {
  ...
  return <>
    <div className="container-wide pt-6"><Breadcrumb items={[{ label: "Vergleich", href: "/software-im-direktvergleich" }]} /></div>
...
function CompareCard({ a, b, line }: { a: Software; b: Software; line: string }) {
  ...
  return <Link href={`/software-im-direktvergleich/${a.slug}-vs-${b.slug}`} className="group flex h-full flex-col overflow-hidden rounded-[20px] border border-[#e2e8f1] bg-white transition-[border-color,box-shadow,transform] hover:-translate-y-0.5 hover:border-[#b8d0f5] hover:shadow-[0_20px_38px_-30px_rgba(6,70,200,.45)]">
