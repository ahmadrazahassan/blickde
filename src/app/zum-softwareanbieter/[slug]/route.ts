...
export async function GET(
  ...
  if (!product) {
    return NextResponse.redirect(new URL("/unternehmenssoftware-vergleichen", serverEnv.siteUrl), 302);
  }

  const target: string | null = product.affiliate_url || product.vendor_website || null;
  if (!target || !/^https:\/\//i.test(target)) {
    return NextResponse.redirect(new URL(`/unternehmenssoftware-vergleichen/${slug}`, serverEnv.siteUrl), 302);
  }
