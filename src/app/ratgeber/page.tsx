...
function FeaturedStory({ article, lead = false, number }: { article: Article; lead?: boolean; number?: number }) {
  return (
    <Link href={`/ratgeber/${article.slug}`} className={cx(
...
function GuideCard({ article }: { article: Article }) {
  return (
    <Link href={`/ratgeber/${article.slug}`} className="group flex h-full min-h-[285px] flex-col rounded-[14px] border border-[var(--color-rule)] bg-white p-6 transition-[border-color,box-shadow,transform] duration-200 hover:-translate-y-1 hover:border-[var(--color-accent)] hover:shadow-[var(--shadow-lift)]">
    ...
    <>
        ...
        <div className="container-page">
          <nav aria-label="Themen">
            <ul className="flex flex-wrap items-center gap-2">
              <li><Link href="/ratgeber" aria-current={!activeTag ? "page" : undefined} className={cx("inline-flex min-h-10 items-center rounded-full border px-4 text-[13px] font-medium transition-colors", !activeTag ? "border-[var(--color-ink)] bg-[var(--color-ink)] text-white" : "border-[var(--color-rule)] text-[var(--color-ink-2)] hover:border-[var(--color-ink)] hover:text-[var(--color-ink)]")}>Alle Themen</Link></li>
              {tags.map((tag) => (
                <li key={tag}><Link href={`/ratgeber?thema=${encodeURIComponent(tag)}`} aria-current={activeTag === tag ? "page" : undefined} className={cx("inline-flex min-h-10 items-center rounded-full border px-4 text-[13px] font-medium transition-colors", activeTag === tag ? "border-[var(--color-ink)] bg-[var(--color-ink)] text-white" : "border-[var(--color-rule)] text-[var(--color-ink-2)] hover:border-[var(--color-ink)] hover:text-[var(--color-ink)]")}>{tag}</Link></li>
              ))}
            </ul>
          </nav>
        </div>
      ...
      <section className="border-t border-[var(--color-rule)] bg-[#fbfaf6] py-14 md:py-20" aria-labelledby="beitraege-heading">
        <div className="container-page">
          ...
          {rest.length === 0 ? (
            <EmptyState title="Zu diesem Thema liegt noch nichts vor" action={<ButtonLink href="/ratgeber" variant="secondary">Alle Beiträge ansehen</ButtonLink>}>Wir schreiben zu einem Thema erst, wenn wir es selbst durchgearbeitet haben.</EmptyState>
          ) : (
      ...
      <section className="border-t border-[var(--color-rule)] bg-white py-12 md:py-16">
        <div className="container-page flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
          ...
          <ButtonLink href="/software" variant="secondary" size="lg">Software entdecken <IconArrowRight size={18} /></ButtonLink>
        </div>
      </section>
    </>
  );
