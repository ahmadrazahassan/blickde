...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: { canonical: `/unternehmenssoftware-vergleichen/${item.slug}` },
    openGraph: {
      title: item.meta_title,
      description: item.meta_description,
      url: `${siteSettings.url}/unternehmenssoftware-vergleichen/${item.slug}`,
      type: "article",
    ...
    <>
      ...
      <header className="container-page flex flex-col items-center pb-9 pt-7 text-center md:pb-12 md:pt-10">
        ...
        <div className="mt-5 flex flex-wrap items-center justify-center gap-2 text-[13px] text-[var(--color-ink-3)]">
          {category ? <Link href={`/software-kategorien/${category.slug}`} className="rounded-[6px] bg-[var(--color-accent-tint)] px-3 py-1.5 font-medium text-[var(--color-primary)]">{category.name}</Link> : null}
          <span>{item.vendor_name}</span><span aria-hidden="true">·</span>
          <Link href={`/unternehmenssoftware-vergleichen/${item.slug}/erfahrungen-und-bewertungen`} className="font-medium text-[var(--color-primary)] hover:underline">{item.review_count > 0 ? `${formatGrade(item.overall_rating, item.review_count)} / 5 · ${reviewCountLabel(item.review_count)}` : "Noch keine Nutzerwertung"}</Link>
        </div>
        <div className="mt-6 flex flex-wrap justify-center gap-2.5">
          ...
          <ButtonLink href={vendorHref(item, `/unternehmenssoftware-vergleichen/${item.slug}`)} external variant="primary" size="md">Zum Anbieter <IconExternal size={16} /></ButtonLink>
          <ButtonLink href={`/unternehmenssoftware-vergleichen/${item.slug}/erfahrungen-und-bewertungen/bewertung-verfassen`} variant="secondary" size="md">Bewertung schreiben</ButtonLink>
        </div>
      ...
      <div className="mx-auto w-[calc(100%-2rem)] max-w-[70rem] pb-16">
          <div className="min-w-0">
            ...
            <section id="einordnung" className="scroll-mt-28 border-b border-[var(--color-rule)] py-16 md:py-20">
              ...
              <div className="mt-7 flex flex-wrap justify-center gap-3"><ButtonLink href={vendorHref(item, `/unternehmenssoftware-vergleichen/${item.slug}`)} external variant="primary">Produkt beim Anbieter prüfen <IconExternal size={16} /></ButtonLink><ButtonLink href={`/software-im-direktvergleich?a=${item.slug}`} variant="secondary">Mit anderer Software vergleichen <IconArrowRight size={16} /></ButtonLink></div>
            </section>
                ...
                <div className={cx("mt-9 grid gap-3 rounded-[26px] bg-[#f5f7fb] p-2.5", item.pricing_plans.length === 2 ? "md:grid-cols-2" : "md:grid-cols-3")}>
                  {item.pricing_plans.map((plan) => (
                    ...
                    >
                      ...
                      <div className="mt-6"><ButtonLink href={vendorHref(item, `/unternehmenssoftware-vergleichen/${item.slug}`)} external variant={plan.highlighted ? "primary" : "secondary"} block>Tarif beim Anbieter ansehen <IconExternal size={16} /></ButtonLink></div>
            ...
            <section id="vergleich" className="scroll-mt-28 border-b border-[var(--color-rule)] py-16 md:py-20">
              ...
              {comparisonAlternative ? <>
                ...
                <div className="mt-6 text-center"><ButtonLink href={`/software-im-direktvergleich/${item.slug}-vs-${comparisonAlternative.slug}`} variant="primary">Vollständigen Vergleich öffnen <IconArrowRight size={18} /></ButtonLink></div>
              </> : <div className="mt-5"><ButtonLink href={`/software-im-direktvergleich?a=${item.slug}`} variant="primary">Programm auswählen <IconArrowRight size={18} /></ButtonLink></div>}
            </section>
            ...
            <section id="bewertungen" className="scroll-mt-24 border-b border-[var(--color-rule)] py-10">
              <SectionHead
                ...
                action={
                  item.review_count > 5 ? (
                    <ButtonLink href={`/unternehmenssoftware-vergleichen/${item.slug}/erfahrungen-und-bewertungen`} variant="secondary">
                      Alle {reviewCountLabel(item.review_count)}
                      <IconArrowRight size={18} />
                    </ButtonLink>
                  ) : (
                    <ButtonLink href={`/unternehmenssoftware-vergleichen/${item.slug}/erfahrungen-und-bewertungen/bewertung-verfassen`} variant="secondary">
                      Bewertung schreiben
                    </ButtonLink>
                  )
                }
            ...
            <section id="alternativen" className="scroll-mt-28 border-b border-[var(--color-rule)] py-16 md:py-20">
              <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="t-micro text-[var(--color-ink-3)]">Weitere Optionen</p><h2 className="t-display-2 mt-4">Alternativen zu {item.name}</h2><p className="mt-3 text-[14px] text-[var(--color-ink-3)]">Programme für eine ähnliche Aufgabe – mit geprüften Preisen und echten Nutzerwertungen.</p></div><Link href={`/unternehmenssoftware-vergleichen/${item.slug}/vergleichbare-alternativen`} className="inline-flex shrink-0 items-center gap-2 text-[14px] font-semibold text-[var(--color-primary)] hover:underline">Alle Alternativen <IconArrowRight size={18} /></Link></div>
            ...
            <p className="mt-8 border-t border-[var(--color-rule)] pt-6 text-center text-[13px] text-[var(--color-ink-3)]">Eine Angabe ist veraltet oder fehlerhaft? <Link href="/kontakt-zur-redaktion" className="font-medium text-[var(--color-primary)] underline underline-offset-2">Korrektur melden</Link></p>
          </div>
...
function AlternativeCard({ item, distribution }: { item: Software; distribution: { stars: number; count: number; share: number }[] }) {
  ...
  return <article className="flex h-full flex-col rounded-[20px] border border-[#dce6f1] bg-white p-5 shadow-[0_18px_34px_-30px_rgba(23,44,60,.35)]">
    ...
    <div className="mt-auto pt-6"><ButtonLink href={`/unternehmenssoftware-vergleichen/${item.slug}`} variant="primary" block>Profil ansehen <IconArrowRight size={16} /></ButtonLink></div>
  </article>;
}
