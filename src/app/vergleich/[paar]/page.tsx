...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: { canonical: `/vergleich/${paar}` },
  };
}
    ...
    <>
      <div className="container-wide pt-6">
        <Breadcrumb
          items={[
            ...
            { label: `${a.name} gegen ${b.name}`, href: `/vergleich/${paar}` },
          ]}
        />
      </div>
      ...
      <section id="praxis" className="section scroll-mt-32 border-b border-[var(--color-rule)]">
        <div className="container-page">
          ...
          <div className="mt-9 grid gap-3 rounded-[26px] bg-[#f5f7fb] p-2.5 md:grid-cols-2">{[a, b].map((item) => <article key={item.id} className="rounded-[20px] border border-[#e2e8f1] bg-white p-7"><div className="flex items-center gap-3"><SoftwareLogo item={item} size={44} /><h3 className="text-[19px] font-semibold text-[var(--color-ink)]">{item.name}</h3></div><p className="mt-5 text-[15px] leading-[1.7] text-[var(--color-ink-2)]">{item.description_full[0]}</p><p className="mt-4 text-[14px] leading-[1.7] text-[var(--color-ink-2)]">{item.description_full[1]}</p><div className="mt-5 border-t border-[#e9edf3] pt-5"><p className="t-micro text-[var(--color-ink-3)]">Wichtige Funktionen</p><ul className="mt-3 space-y-2">{item.top_features.slice(0, 3).map((feature) => <li key={feature} className="flex gap-2 text-[13px] text-[var(--color-ink-2)]"><IconCheck size={16} className="shrink-0 text-[var(--color-primary)]" />{feature}</li>)}</ul></div><div className="mt-6"><ButtonLink href={`/software/${item.slug}`} variant="secondary">Ausführliches Profil lesen</ButtonLink></div></article>)}</div>
        </div>
      </section>
      ...
      <section id="urteil" className="section scroll-mt-32">
        <div className="container-page">
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            <ButtonLink href={`/software/${a.slug}`} variant="secondary">
              Profil von {a.name}
            </ButtonLink>
            <ButtonLink href={`/software/${b.slug}`} variant="secondary">
              Profil von {b.name}
            </ButtonLink>
            <ButtonLink href="/vergleich" variant="quiet">
      ...
      {related.length ? <section aria-labelledby="weitere-vergleiche" className="section border-t border-[var(--color-rule)]">
        <div className="container-page">
          <div className="flex flex-wrap items-end justify-between gap-4"><div><p className="t-micro text-[var(--color-primary)]">Weiterlesen</p><h2 id="weitere-vergleiche" className="t-display-2 mt-3">Weitere Vergleiche für Ihre Auswahl</h2></div><ButtonLink href="/vergleich" variant="quiet">Alle Vergleiche ansehen</ButtonLink></div>
          <div className="mt-8 grid gap-3 sm:grid-cols-2">{related.map((entry) => entry.left && entry.right ? <Link key={entry.pair} href={`/vergleich/${entry.pair}`} className="group rounded-[18px] border border-[#e0e8f2] bg-white p-5 transition-colors hover:border-[#9bbbf0]"><div className="flex items-center gap-3"><SoftwareLogo item={entry.left} size={34} /><span className="text-[12px] font-semibold text-[var(--color-ink-3)]">oder</span><SoftwareLogo item={entry.right} size={34} /></div><h3 className="mt-4 text-[17px] font-semibold text-[var(--color-ink)]">{entry.left.name} oder {entry.right.name}</h3><p className="mt-2 line-clamp-2 text-[13px] leading-[1.6] text-[var(--color-ink-3)]">{entry.headline}</p><span className="mt-4 inline-block text-[13px] font-semibold text-[var(--color-primary)] group-hover:underline">Vergleich lesen →</span></Link> : null)}</div>
        </div>
      </section> : null}
    </>
  );
  ...
  return (
    <Card className="p-6">
      ...
      <div className="mt-6 border-t border-[var(--color-rule)] pt-5">
        <ButtonLink href={`/software/${item.slug}`} variant="secondary" size="sm">
